...
\citep{Hernandez2018} plantea que la fundamentación teórica es una parte esencial de la
...
\newcommand{\huestereotipo}[3]{%
  ...
  \multicolumn{2}{|l|}{\textbf{Estereotipo:}} \\
  \hline
...
\hline
\huestereotipo{Images/Iniciar - Initial.jpg}{fig:hu01_estereotipo}{Estereotipo de la HU-01:
...
% ── HU-02 ─────────────────────────────────────────────────────────────────────
\begin{longtable}[c]{|p{3.5cm}|p{10cm}|}
\caption{HU-02 Iniciar proceso de subida fragmentada}
\label{tb:hu02}\\
\hline
\huhead{HU-02}{18 h}{1}{Iniciar proceso de subida fragmentada}
\textbf{Prioridad de Negocio:} Crítica & \textbf{Asignado a:} Xavier Verdecie \\
\hline
\multicolumn{2}{|l|}{\textbf{Descripción:}} \\
\hline
...
\multicolumn{2}{|l|}{\textbf{Observaciones:}} \\
\hline
\multicolumn{2}{|p{13.5cm}|}{Esta HU implementa el núcleo del protocolo de subida fragmentada sobre
Django REST Framework, utilizando minio-py v7.2.18. La lógica de partición reside en Angular v15.} \\
\hline
\huestereotipo{Images/Iniciar - Initial.jpg}{fig:hu02_estereotipo}{Estereotipo de la HU-02:
...
\hline
\huestereotipo{Images/Pausar - Initial.jpg}{fig:hu03_estereotipo}{Estereotipo de la HU-03:
...
\hline
\huestereotipo{Images/Reanudar - Initial.jpg}{fig:hu04_estereotipo}{Estereotipo de la HU-04:
...
\hline
\huestereotipo{Images/Pausar - Initial.jpg}{fig:hu05_estereotipo}{Estereotipo de la HU-05:
...
\hline
\huestereotipo{Images/Lista de subidas Bien - Initial.jpg}{fig:hu06_estereotipo}{Estereotipo de
...
\huestereotipo{Images/Lista de subidas sin publicar - Initial.jpg}{fig:hu07_estereotipo}{%
  Estereotipo de la HU-07: Lista de gestión mostrando videos completados en estado ``Sin Procesar''
...
\huestereotipo{Images/Lista de subidas sin publicar Monit - Initial.jpg}{fig:hu08_estereotipo}{%
  Estereotipo de la HU-08: Lista de gestión mostrando videos en proceso de transcodificación con
...
\huestereotipo{Images/Lista de subidas publicadas - Initial.jpg}{fig:hu09_estereotipo}{%
  Estereotipo de la HU-09: Lista de gestión mostrando videos procesados con botón ``Ver'' para
...
\huestereotipo{Images/Lista de subidas Bien - Initial.jpg}{fig:hu10_estereotipo}{%
  Estereotipo de la HU-10: Lista de gestión con acción de eliminar disponible para subidas
...
especifican \textit{lo que} un sistema debe hacer, los RNF definen \textit{qué tan bien} debe
...
\hline
HU-02 & Iniciar proceso de subida fragmentada & 18 & 1 \\
\hline
...
El esfuerzo total estimado es de \textbf{102 horas de desarrollo}, distribuidas en 3 sprints. 
...
se obtiene una duración de \textbf{10.2 semanas} para el proyecto completo, redondeando a 
\textbf{11 semanas} para mantener márgenes de maniobra ante imprevistos.
...
& HU-02: Iniciar proceso de subida fragmentada & \\
...
\hline
\multicolumn{2}{|c|}{\textbf{Duración total del proyecto}} & \textbf{10.2 semanas} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Sprint 1 (54h / 5.4 semanas):} Establece la funcionalidad core de subida fragmentada 
...
Este patrón constituye el aporte arquitectónico central propuesto en la investigación. En el flujo 
monolítico original de Picta, la subida del video y su renderizado ocurrían de forma síncrona dentro 
de la misma transacción HTTP, lo que mantenía la conexión del cliente abierta durante todo el proceso 
de transcodificación, consumiendo recursos del servidor de forma continua y exponiéndose a timeouts 
ante conexiones inestables.

La arquitectura dirigida por eventos propuesta desacopla estos dos procesos \citep{Richards2020} mediante la 
emisión de un evento de dominio al completarse la subida y la carga de metadatos. Un \textit{message 
broker} o corredor de mensajes es un patrón arquitectónico que permite la comunicación asíncrona entre 
componentes mediante el intercambio de mensajes a través de colas \citep{Hohpe2003}. En sistemas distribuidos, 
el broker actúa como intermediario que garantiza la entrega de eventos incluso ante fallos temporales 
de los consumidores.

Para la implementación del desacoplamiento, se propone la integración de \textbf{Celery} 
\citep{CeleryDocs2024}, un sistema distribuido de colas de tareas para Python ampliamente adoptado en 
aplicaciones Django. Celery utiliza un broker de mensajes subyacente ---típicamente Redis o RabbitMQ--- 
para gestionar la cola de tareas pendientes. Se selecciona \textbf{Redis} \citep{Redis2024} como broker 
por las siguientes razones:

\begin{enumerate}
  \item \textbf{Rendimiento:} Redis es una base de datos en memoria que ofrece latencias submilisegundo 
  para operaciones de lectura/escritura, lo que garantiza tiempos de respuesta mínimos en la emisión 
  y consumo de eventos.
  
  \item \textbf{Simplicidad operacional:} A diferencia de RabbitMQ, que requiere configuración de 
  exchanges, bindings y políticas de persistencia, Redis proporciona operaciones de cola mediante 
  estructuras de datos nativas (listas, sorted sets) con una curva de aprendizaje menor.
  
  \item \textbf{Ecosistema Django:} Redis ya se utiliza en Picta para gestión de sesiones y caché, 
  por lo que su adopción como broker no introduce dependencias adicionales al stack tecnológico.
  
  \item \textbf{Compatibilidad con Celery:} Redis es el broker recomendado por la documentación oficial 
  de Celery para aplicaciones de baja a media escala \citep{CeleryDocs2024}, soportando completamente 
  el protocolo de acknowledgment y retry de mensajes.
\end{enumerate}

En la implementación desarrollada, el endpoint \texttt{complete} del \texttt{MultipartUploadViewSet} 
finaliza la subida multipart en MinIO y actualiza el estado de la sesión a \texttt{completed}, 
liberando inmediatamente la conexión HTTP:

\begin{lstlisting}[language=Python, caption={Finalización de subida en MultipartUploadViewSet}]
...
def complete(self, request, pk=None):
    ...
    for part_dict in parts_json:
        etag_limpio = part_dict['ETag'].strip('"')
        uploaded_parts_list.append(
            Part(part_number=part_dict['part_number'],
                 etag=etag_limpio)
        )
    
    client._complete_multipart_upload(
        upload.bucket_name, upload.file_name,
        upload.upload_id, uploaded_parts_list
    )
    
    upload.status = 'completed'
    upload.parts = parts_for_db
    upload.save(update_fields=['status','parts','updated_at'])
    
    return Response({"status": "completed"}, 
                    status=status.HTTP_200_OK)
\end{lstlisting}

Una vez persistido el estado \texttt{completed}, la arquitectura propuesta contempla la emisión de 
una tarea Celery que será consumida de forma asíncrona por un worker independiente. Este worker 
ejecutará el pipeline de renderizado y transcodificación sin ninguna dependencia del ciclo de vida 
de la conexión HTTP original. La integración se realizará mediante la invocación de una tarea Celery 
al finalizar el método \texttt{complete}:
...
De esta forma, el servidor web queda liberado para atender nuevas solicitudes inmediatamente después 
de confirmar la recepción del evento, reduciendo sustancialmente la carga sostenida sobre los recursos 
del servidor.
...
La propuesta de solución define un módulo de subida fragmentada que integra seis funcionalidades 
...
integración \textit{seamless} \citep{Zhuk2004} con la arquitectura de autorización en producción.
