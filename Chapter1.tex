...
\begin{description}
\sloppy
  \item[Fragmentación:] se refiere a la descomposición de un objeto de datos (como un archivo de video de gran tamaño) 
  en unidades menores e independientes, denominadas fragmentos o chunks, para facilitar su transmisión y procesamiento 
  ...
  de cada fragmento de video durante el proceso de carga fragmentada a servicios de almacenamiento de datos.
  ...
  \item[Renderizado:] Es el proceso final del ciclo de vida de una petición, donde los datos estructurados (metadatos) 
  pasan de ser objetos internos del servidor a una representación externa legible por el cliente (generalmente JSON o XML).

  \item[Desacoplamiento:] Se refiere al grado de independencia entre los componentes de un sistema. En ingeniería de software, 
  un sistema altamente desacoplado es aquel en el que los módulos pueden realizar su función, evolucionar y escalar sin que un 
  cambio o fallo en uno afecte directamente al funcionamiento interno del otro. Gracias al uso de S3 Event Notifications y Celery, 
  el proceso de subida no está atado al procesamiento de metadatos.
...
\begin{figure}[H]
  ...
  \caption{Flujo monolítico y síncrono del proceso de publicación en Picta. Elaboración Propia}
...
Asimismo, la elección tecnológica se fundamenta en la capacidad de los componentes seleccionados para 
soportar flujos de trabajo asíncronos y arquitecturas desacopladas, requisitos indispensables para la 
gestión eficiente de grandes volúmenes de datos mediante subidas fragmentadas. Se busca no solo garantizar 
la operatividad técnica inmediata, sino también facilitar la escalabilidad futura y el mantenimiento evolutivo 
del sistema dentro del entorno de producción de la plataforma, asegurando una integración fluida con los servicios 
de infraestructura y almacenamiento de objetos ya desplegados.
...
de carga fragmentada con arquitectura dirigida por eventos sobre una plataforma en producción---
